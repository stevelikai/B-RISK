...
error_derived:
    ...
    Sub EndPoint_Flags(ByRef tim As Double, ByRef Target As Double, ByRef UT As Double, ByRef Visibility1 As Double, ByRef sprink As Double, ByRef layer As Double, ByRef LT As Double, ByRef hrr As Double)
        ...
        Static ConvectFlag, TargetFlag, TempFlag, VisibilityFlag As Short
        ...
        If Target >= TargetEndPoint And TargetFlag = 0 Then
            frmEndPoints.lblTarget.Text = "FED Thermal (incap) Exceeds " & VB6.Format(TargetEndPoint) & " at " & VB6.Format(tim, "0.0") & " Seconds."
            TargetFlag = 1
        End If

        If UT >= TempEndPoint And TempFlag = 0 Then
            frmEndPoints.lblTemp.Text = "Upper Layer Temperature Exceeds " & VB6.Format(TempEndPoint - 273) & " deg C at " & VB6.Format(tim, "0.0") & " Seconds."
            TempFlag = 1
        End If
...
Prophandler:
    ...
    Function MassLoss_ObjectwithCLT(ByVal id As Integer, ByVal tim As Double, ByRef mburner As Double, ByRef mFloor As Double, ByRef mWall As Double, ByRef mCeiling As Double) As Double
        ...
        MassLoss_ObjectwithCLT = MassLoss_Total_pluswood(tim, mWall, mCeiling)

    End Function

    Function MassLoss_ObjectwithFuelResponse(ByVal id As Integer, ByVal tim As Double, ByRef Qburner As Double, ByRef QFloor As Double, ByRef QWall As Double, ByRef QCeiling As Double, ByVal O2lower As Double, ByVal ltemp As Double, ByVal mplume As Double, ByVal incidentflux As Double, ByRef burningrate As Double) As Double
        '*  ===================================================================
...
here:
    ...
    Function MassLoss_Object(ByVal id As Integer, ByVal tim As Double, ByRef Qburner As Double, ByRef QFloor As Double, ByRef QWall As Double, ByRef QCeiling As Double) As Double
        ...
        If useCLTmodel = True And frmCLT.chkWoodIntegralModel.Checked = True Then
            'using CLT integral model
            MassLoss_Object = MassLoss_ObjectwithCLT(id, tim, Qburner, QFloor, QWall, QCeiling)
            Exit Function
        ElseIf useCLTmodel = True And g_post = False And frmCLT.chkWoodIntegralModel.Checked = False Then
    ...
    Function MassLoss_Total_pluswood(ByVal T As Double, ByRef mwall As Double, ByRef mceiling As Double) As Double
        ...
        Static wood_consumed As Double 'kg/m2
        'If T = flashover_time + Timestep Then wood_consumed = 0 'initialise
        Dim thickness_mass As Double = 0.105 'm thick of wood
        Dim MLR_wood As Double

        If thickness_mass > 0 Then
            ...
            'Dim Tign As Double = 384 'C igntion temp
            ...
            Dim hc As Double = 0.01 'kW/m2K


            Dim Qinc As Double = QCeilingAST(fireroom, 0, T / Timestep) 'kW/2 incident radiant heat flux
            ...
            Dim TfromIgn As Double = T - flashover_time 's

            If T > 40 * 60 Then
                'TfromIgn = T - 40 * 60  'layer delaminates at 40 min
            End If
            ...
            'MLR_longtime = MLR_longtime * 2 'double it to test
            ...
            'MLR_wood = Min(MLR_wood, 30) 'g/s/m2 capped at 30g/s/m2

            'Dim mass_wood As Double = dw * thickness_mass 'kg/m2 initial mass of wood per sqm
            'Dim mass_wood_delta As Double = MLR_wood * (Timestep) / 1000 'kg/m2
            'wood_consumed = wood_consumed + mass_wood_delta 'cumulative wood consumed kg/m2
            'If wood_consumed > mass_wood Then MLR_wood = 0 'out of fuel from surface linings

        End If

        'max area for now, can make more accurate
        Dim surfacearea As Double = RoomFloorArea(fireroom) + (RoomLength(fireroom) + RoomWidth(fireroom)) * 2 * RoomHeight(fireroom) 'm2


        'if clt model then update this value
        If useCLTmodel = True Then
            'mass = fuelmasswithCLT
        End If
        ...
        If Flashover = True And g_post = True Then
            ...
            If thickness_mass > 0 Then
                ...
                mceiling = RoomFloorArea(fireroom) * MLR_wood / 1000 'kg/s
                mwall = (RoomLength(fireroom) + RoomWidth(fireroom)) * 2 * RoomHeight(fireroom) * MLR_wood / 1000 'kg/s
            End If

        End If

        MassLoss_Total_pluswood = total + wood_MLR
    ...
    Sub New_File()
        ...
        frmOptions1.txtErrorControl.Text = VB6.Format(Error_Control, "0.000")
        frmOptions1.txtErrorControlVentFlow.Text = VB6.Format(Error_Control_ventflow, "0.000000")
        ...
        For i = 1 To NumberObjects
            ...
            If frmOptions1.optRCNone.Checked = True Or i > 1 Then
                ...
                If sootmode = True Then 'manual entry of pre post flashover soot yields
                    ...
                    If Flashover = True Then
                        'in this case Qburner, QFloor, QWall, QCeiling are all zero
                        dummy = MassLoss_ObjectwithFuelResponse(i, T, Qburner, QFloor, QWall, QCeiling, o2lower, ltemp, mplume, incidentflux, 0) * postSoot
                    Else
                        ...
                        Else
                            'in this case Qburner, QFloor, QWall, QCeiling are all zero
                            dummy = MassLoss_ObjectwithFuelResponse(i, T, Qburner, QFloor, QWall, QCeiling, o2lower, ltemp, mplume, incidentflux, 0) * preSoot
                ...
                Else
                    ...
                    'in this case Qburner, QFloor, QWall, QCeiling are all zero
                    dummy = MassLoss_ObjectwithFuelResponse(i, T, Qburner, QFloor, QWall, QCeiling, o2lower, ltemp, mplume, incidentflux, 0) * SootYield(i)
            ...
            Else
                'room corner simulation with flame spread
                'may need checking, does this work properly with manual input of soot yields?
                mlo = MassLoss_ObjectwithFuelResponse(i, T, Qburner, QFloor, QWall, QCeiling, o2lower, mplume, ltemp, incidentflux, 0)
                If WallEffectiveHeatofCombustion(fireroom) > 0 And CeilingEffectiveHeatofCombustion(fireroom) > 0 Then
    ...
    Public Sub view_output(ByRef filename As String)
        ...
        If NumberTimeSteps <> 0 Then
            ...
            For k = 1 To howmany + 1
                ...
                X = Y
                room = 1
                If frmprintvar.ChkCJtemp.CheckState = System.Windows.Forms.CheckState.Checked Then Print(1, TAB(10), "Ceiling Jet Temp at Link (C)", TAB(X), VB6.Format(CJetTemp(j, 1, 0) - 273, "0.0"))
                For room = 2 To NumberRooms
                    X = X + 10
                    If frmprintvar.ChkCJtemp.CheckState = System.Windows.Forms.CheckState.Checked And room = fireroom Then Print(1, TAB(X), VB6.Format(CJetTemp(j, 1, 0) - 273, "0.0"))
                Next room
